...
pub async fn get_tweet(auth: Oauth1aToken, id: NumericId) -> Option<Tweet> {
...
pub async fn get_user(auth: Oauth1aToken, id: NumericId) -> Option<User> {
	let api = TwitterApi::new(auth);
...
pub async fn get_my_user(auth: Oauth1aToken) -> Option<User> {
	let id: u64 = dotenv::var("USER_ID").unwrap().parse().unwrap();
	get_user(auth, NumericId::new(id)).await
}

pub async fn get_user_by_name(auth: Oauth1aToken, username: String) -> User {
